...
(function($){
	...
	function scrollTo(target, value){
		...
		if (item.length){
			if (item.position().top <= 0){ var h="panel.scrollTop()" + item.position().top; panel.scrolltop(h); } else if (item.position().top item.outerheight()> panel.height()){
	...
	function setValues(target, values, remainText){
		...
		var theRow = null;
		var vv = [], ss = [];
		for(var i=0; i<values.length; i++){ var v="values[i];" s="v;" row="opts.finder.getRow(target," v); if (row){ therow="row;" el="opts.finder.getEl(target," (!el.hasclass('combobox-item-selected')){ el.addclass('combobox-item-selected'); opts.onselect.call(target, row); } else { opts.mappingrows) || v; vv.push(v); ss.push(s); (!remaintext){ $(target).combo('settext', ss.join(opts.separator)); (opts.showitemicon){ tb="$(target).combobox('textbox');" tb.removeclass('textbox-bgicon ' + opts.textboxiconcls); (therow && therow.iconcls){ tb.addclass('textbox-bgicon therow.iconcls); opts.textboxiconcls="theRow.iconCls;" $(target).combo('setvalues', vv); panel.triggerhandler('scroll'); trigger the group sticking function findtext(value, a){ item="$.easyui.getArrayItem(a," opts.valuefield, value); return ? item[opts.textfield] : undefined; ** * load data, old list items will be removed. loaddata(target, remaintext){ state="$.data(target," 'combobox'); opts="state.options;" state.data="opts.loadFilter.call(target," data); opts.view.render.call(opts.view, target, $(target).combo('panel'), state.data); vv="$(target).combobox('getValues');" $.easyui.foreach(state.data, false, function(row){ (row['selected']){ $.easyui.addarrayitem(vv, row[opts.valuefield]+''); }); (opts.multiple){ setvalues(target, vv, remaintext); vv.length [vv[vv.length-1]] [], opts.onloadsuccess.call(target, request remote data url property is setted. request(target, url, param, 'combobox').options; (url){ opts.url="url;" param="$.extend({}," opts.queryparams, param||{}); {}; (opts.onbeforeload.call(target, param)="=" false) return; opts.loader.call(target, function(data){ }, function(){ opts.onloaderror.apply(this, arguments); do query action doquery(target, q){ highlightitem="$();" qq="opts.multiple" q.split(opts.separator) [q]; (opts.mode="=" 'remote'){ _setvalues(qq); null, {q:q}, true); panel="$(target).combo('panel');" panel.find('.combobox-item-hover').removeclass('combobox-item-hover'); panel.find('.combobox-item,.combobox-group').hide(); $.map(qq, function(q){ q="$.trim(q);" value="q;" for(var i="0;" i<data.length; (opts.filter.call(target, q, row)){ g="row[opts.groupField];" v).show(); (s.tolowercase()="=" q.tolowercase()){ (opts.reversed){ select(target, v, (opts.groupfield !="g){" opts.finder.getgroupel(target, g).show(); vv.push(value); _setvalues(vv); _setvalues(vv){ highlightitem.addclass('combobox-item-hover'); opts.multiple (q?vv:[]) doenter(target){ t="$(target);" (item.length){ item.removeclass('combobox-item-hover'); item); (item.hasclass('combobox-item-selected')){ t.combobox('unselect', t.combobox('select', $.map(t.combobox('getvalues'), function(v){ (getrowindex(target, v)>= 0){
				vv.push(v);
			}
		});
	...
	var defaultView = {
		render: function(target, container, data){
			...
			var dd = [];
			var group = undefined;
			for(var i=0; i<data.length; 1 i++){ var row="data[i];" v="row[opts.valueField]+'';" s="row[opts.textField];" g="row[opts.groupField];" if (g){ (group !="g){" group="g;" state.groups.push({ value: g, startindex: i, count: }); dd.push('<div id="' + (state.groupIdPrefix+'_'+(state.groups.length-1)) + '" class="combobox-group">');
