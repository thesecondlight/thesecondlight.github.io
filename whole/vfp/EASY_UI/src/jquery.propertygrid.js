...
(function($){
	...
	var groupview = $.extend({}, $.fn.datagrid.defaults.view, {
		render: function(target, container, frozen){
			...
			for(var i=0; i<groups.length; i++){ table.push(this.rendergroup.call(this, target, i, groups[i], frozen)); } $(container).html(table.join('')); }, rendergroup: function(target, groupindex, group, frozen){ var state="$.data(target," 'datagrid'); opts="state.options;" fields="$(target).datagrid('getColumnFields'," frozen); table="[];" table.push('<div class="datagrid-group" group-index=" + groupIndex + ">');
			...
			for(var j=0; j<group.rows.length; 2 j++) { var css="opts.rowStyler" ? opts.rowstyler.call(target, index, group.rows[j]) : ''; classvalue ; stylevalue if (typeof 'string'){ } else (css){ || cls="class=" datagrid-row " + (index % && opts.striped 'datagrid-row-alt ' ') '"'; style="styleValue" 'style="' + styleValue + '" rowid="state.rowIdPrefix" '-' (frozen?1:2) index; table.push('<tr id="' + rowId + '" datagrid-row-index="' + index + '">');
				table.push(this.renderRow.call(this, target, fields, frozen, index, group.rows[j]));
				table.push('');
				index++;
			}
			table.push('</group.rows.length;></tbody></table>');
			return table.join('');
		},
		...
		onBeforeRender: function(target, rows){
			...
			var groups = [];
			for(var i=0; i<rows.length; i++){ var row="rows[i];" group="getGroup(row[opts.groupField]);" if (!group){ value: row[opts.groupfield], rows: [row] }; groups.push(group); } else { group.rows.push(row); index="0;" newrows="[];" for(var i="0;" i<groups.length; group.startindex="index;" +="group.rows.length;" state.data.rows="newRows;" this.groups="groups;" that="this;" settimeout(function(){ that.bindevents(target); },0); function getgroup(value){ (group.value="=" value){ return group; null; initcss(){ (!$('#datagrid-group-style').length){ $('head').append( '<style id="datagrid-group-style">' +
	...
	$.extend(groupview, {
		...
		insertRow: function(target, index, row){
			...
			for(var i=0; i<this.groups.length; i++){ if (this.groups[i].value="=" row[opts.groupfield]){ group="this.groups[i];" groupindex="i;" break; } (group){ (index="=" undefined || index="=" null){ < group.startindex){ else> group.startIndex + group.rows.length){
		...
		deleteRow: function(target, index){
			...
			if (group.rows.length > 1){
				...
				for(var i=groupIndex+1; i</this.groups.length;></rows.length;></groups.length;>