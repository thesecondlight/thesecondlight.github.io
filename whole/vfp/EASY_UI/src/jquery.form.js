...
(function($){
	...
	function load(target, data){
		...
		function _loadBox(name, val){
			...
			if (field.length){
				for(var i=0; i<opts.fieldtypes.length; i++){ var type="opts.fieldTypes[i];" state="field.data(type);" if (state){ (state.options.multiple || state.options.range){ field[type]('setvalues', val); } else { field[type]('setvalue', return true; false; ** * clear the form fields function clear(target){ $('input,select,textarea', target).each(function(){ ($(this).hasclass('textbox-value')){return;} t="this.type," tag="this.tagName.toLowerCase();" (t="=" 'text' 'hidden' 'password' 'textarea'){ this.value ; 'file'){ file="$(this);" (!file.hasclass('textbox-value')){ newfile="file.clone().val('');" newfile.insertafter(file); (file.data('validatebox')){ file.validatebox('destroy'); newfile.validatebox(); file.remove(); 'checkbox' 'radio'){ this.checked="false;" (tag="=" 'select'){ this.selectedindex="-1;" }); tmp="$();" opts="$.data(target," 'form').options; for(var i="0;" i<opts.fieldtypes.length; field="form.find('.'+type+'-f').not(tmp);" (field.length && field[type]){ field[type]('clear'); form.form('validate'); reset(target){ target.reset();>=0; i--){
