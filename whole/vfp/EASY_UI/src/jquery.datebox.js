...
(function($){
	...
	function createBox(target){
		...
		function setButtons(target){
			...
			for(var i=0; i<opts.buttons.length; i++){ var td="$('<td">').appendTo(tr);
				var btn = opts.buttons[i];
				var t = $('<a class="datebox-button-a" href="javascript:;" target="_blank" rel="noopener"></a>').html($.isFunction(btn.text) ? btn.text(target) : btn.text).appendTo(td);
