...
(function($){
	...
	function showSelectMenus(target){
		...
		if ($(target).find('.calendar-menu-month-inner').is(':empty')){
			...
			for(var i=0; i<3; i++){ var tr="$('<tr">').appendTo(t);
				for(var j=0; j<4; j++){ $('<td class="calendar-nav calendar-menu-month">').html(opts.months[idx++]).attr('abbr',idx).appendTo(tr);
				}
			}
		}
	...
	function getWeeks(target, year, month){
		...
		for(var i=1; i<=lastday; i++) dates.push([year,month,i]); group date by week var weeks="[]," memoday="-1;" while(dates.length> 0){
		...
		if (firstWeek.length < 7){
			...
			var firstDate = firstWeek[0];
			var week = [];
			for(var i=1; i<=7; i++){ var date="new" date(firstdate[0], firstdate[1]-1, firstdate[2]-i); week.unshift([date.getfullyear(), date.getmonth()+1, date.getdate()]); } weeks.unshift(week); lastweek="weeks[weeks.length-1];" while(lastweek.length < 7){ lastdate="lastWeek[lastWeek.length-1];" date(lastdate[0], lastdate[1]-1, lastdate[2]+1); lastweek.push([date.getfullyear(), if (weeks.length 6){ week="[];" for(var i="1;" i<="7;" lastdate[2]+i); week.push([date.getfullyear(), weeks.push(week); return weeks; ** * show the calendar day. function show(target){ opts="$.data(target," 'calendar').options; (opts.current && !opts.validator.call(target, opts.current)){ opts.current="null;" now="new" date(); todayinfo="now.getFullYear()+','+(now.getMonth()+1)+','+now.getDate();" currentinfo="opts.current" ? (opts.current.getfullyear()+','+(opts.current.getmonth()+1)+','+opts.current.getdate()) : ''; calulate saturday and sunday index saindex="6" - opts.firstday; suindex="saIndex" + 1; (saindex>= 7) saIndex -= 7;
		...
		for(var i=opts.firstDay; i<opts.weeks.length; i++){ data.push('<th>'+opts.weeks[i]+'');
		}
		for(var i=0; i<opts.firstday; i++){ data.push('<th>'+opts.weeks[i]+'');
		}
		data.push('</opts.firstday;></opts.weeks.length;></tr></thead>');
		
		data.push('<tbody>');
		var weeks = getWeeks(target, opts.year, opts.month);
		for(var i=0; i<weeks.length; i++){ var week="weeks[i];" cls ; if (i="=" 0){cls="calendar-first" ;} else weeks.length - 1){cls="calendar-last" data.push('<tr class="' + cls + '">');
			if (opts.showWeek){
			...
			for(var j=0; j<week.length; j++){ var day="week[j];" s="day[0]+','+day[1]+','+day[2];" dvalue="new" date(day[0], parseint(day[1])-1, day[2]); d="opts.formatter.call(target," dvalue); css="opts.styler.call(target," classvalue ; stylevalue if (typeof 'string'){ } else (css){ || ''; cls="calendar-day" (!(opts.year="=" day[0] && opts.month="=" day[1])){ +=" calendar-other-month" (s="=" todayinfo){cls ;} currentinfo){cls (j="=" saindex){cls suindex){cls 0){cls week.length-1){cls classvalue; (!opts.validator.call(target, dvalue)){ data.push('<td class="' + cls + '" abbr="' + s + '" style="' + styleValue + '">' + d + '');
			}
			data.push('');
		}
		data.push('</week.length;></weeks.length;></tbody>');
		data.push('</table>');
