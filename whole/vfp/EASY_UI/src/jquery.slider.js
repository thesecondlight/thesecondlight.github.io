...
(function($){
	function init(target){
		var slider = $('<div class="slider">' +
				...
				'<a href="javascript:;" class="slider-handle" target="_blank" rel="noopener"></a>' +
	...
	function showRule(target){
		...
		function _build(aa){
			...
			rule.empty();
			label.empty();
			for(var i=0; i<aa.length; i++){ var distance="i*100/(aa.length-1)+'%';" span="$('<span">').appendTo(rule);
		...
		function setPos(pos, handle){
			...
			if (opts.range){
				...
				if (handle){
					...
					if (value <= v2 && isleft){ v1="value;" } else if (value>= v1 && (!isLeft)){
						v2 = value;
					}
				} else {
					if (value < v1){
	...
	function setValues(target, values){
		...
		for(var i=0; i<values.length; i++){ var value="values[i];" if (value < opts.min)> opts.max) value = opts.max;
