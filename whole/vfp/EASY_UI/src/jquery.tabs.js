	...
	function addTools(container){
		...
		if (opts.tools) {
			if (typeof opts.tools == 'string'){
				...
				for(var i=0; i<opts.tools.length; i++){ var td="$('<td">').appendTo(tr);
					var tool = $('<a href="javascript:;" target="_blank" rel="noopener"></a>').appendTo(td);
...
//					selectTab(container, getLiIndex(li));
	...
	function createTab(container, options, pp) {
		...
		var tab = $(
				'<li>' +
				'<a href="javascript:;" class="tabs-inner" target="_blank" rel="noopener">' +
	...
	function updateTab(container, param){
		...
		if (param.type == 'all' || param.type == 'header'){
			...
			if (opts.header){
				...
				if (opts.closable){
					...
					$('<a href="javascript:;" class="tabs-close" target="_blank" rel="noopener"></a>').appendTo(tab);
				} else{
				...
				if (opts.tools){
					...
					if ($.isArray(opts.tools)){
						p_tool.empty();
						for(var i=0; i<opts.tools.length; i++){ var t="$('<a" href="javascript:;">').appendTo(p_tool);
			...
			// 	for(var i=0; i<selecthis.length; i++){ if (selecthis[i]="=" oldtitle){ selecthis[i]="opts.title;" } (opts.disabled){ opts.tab.addclass('tabs-disabled'); else { opts.tab.removeclass('tabs-disabled'); setsize(container); $.data(container, 'tabs').options.onupdate.call(container, opts.title, gettabindex(container, pp)); ** * close a tab with specified index or title function closetab(container, which) var state="$.data(container," 'tabs'); opts="state.options;" tabs="state.tabs;" selecthis="state.selectHis;" (!exists(container, which)) return; which); tab); (opts.onbeforeclose.call(container, title, index)="=" false) which, true); tab.panel('options').tab.remove(); tab.panel('destroy'); opts.onclose.call(container, index); setscrollers(container); remove the select history item his="[];" for(var i="0;" i<selecthis.length; tindex="selectHis[i];" (tindex !="index){" his.push(tindex> index ? tindex-1 : tindex);
			}
		}
		state.selectHis = his;
		...
		// for(var i=0; i<selecthis.length; i++){ if (selecthis[i]="=" title){ selecthis.splice(i, 1); i --; } select the nearest tab panel var histitle="selectHis.pop();" (histitle){ selecttab(container, histitle); else (tabs.length){ 0); ** * get specified function gettab(container, which, removeit){ tabs="$.data(container," 'tabs').tabs; (typeof which="=" 'number'){ (which>=0 && which < tabs.length){
				tab = tabs[which];
				if (removeit){
					tabs.splice(which, 1);
				}
			...
			var tmp = $('<span></span>');
			for(var i=0; i<tabs.length; i++){ var p="tabs[i];" tmp.html(p.panel('options').title); title="tmp.text();" tmp.html(which); which="tmp.text();" if (title="=" which){ tab="p;" (removeit){ tabs.splice(i, 1); } break; tmp.remove(); return tab; function gettabindex(container, tab){ tabs="$.data(container," 'tabs').tabs; for(var i="0;" i<tabs.length; (tabs[i][0]="=" $(tab)[0]){ i; -1; getselectedtab(container){ (tab.panel('options').tab.hasclass('tabs-selected')){ null; ** * do first select action, no is setted the will be selected. dofirstselect(container){ state="$.data(container," 'tabs') opts="tabs[i].panel('options');" (opts.selected && !opts.disabled){ selecttab(container, i); return; state.options.selected); which); (p !p.is(':visible')){ stopanimate(container); (!p.panel('options').disabled){ p.panel('open'); unselecttab(container, p.is(':visible')){ p.panel('close'); stopanimate(container){ $(container).children('div.tabs-panels').each(function(){ $(this).stop(true, true); }); exists(container, gettab(container, which) !="null;" showheader(container, visible){ 'tabs').options; opts.showheader="visible;" $(container).tabs('resize'); showtool(container, tool="$(container).find('">.tabs-header>.tabs-tool');
		if (visible){
...
</tabs.length;></selecthis.length;></selecthis.length;></opts.tools.length;></opts.tools.length;>