...
(function($){
	...
	function init(target){
		...
		for(var i=0; i<menus.length; i++){ createmenu(target, menus[i]); } function splitmenu(menu){ var menus="[];" menu.addclass('menu'); menus.push(menu); if (!menu.hasclass('menu-content')){ menu.children('div').each(function(){ submenu="$(this).children('div');" (submenu.length){ submenu.appendto('body'); this.submenu="submenu;" point to the sub menu mm="splitMenu(submenu);" }); return menus; div){ (!menu.data('menu')){ menu.data('menu', { options: $.parser.parseoptions(menu[0], ['width','height']) createitem(target, this); $('<div class="menu-line">').prependTo(menu);
		}
