...
var _11=$("<div class="\" tabs-tool\""><table cellspacing="\" 0\"" cellpadding="\" style="\" height:100%\""><tr></tr></table></div>").appendTo(_10);
var tr=_11.find("tr");
for(var i=0;i<_f.tools.length;i++){ var td="$(" <td">").appendTo(tr);
...
if(_3e.index<0){ _3e.index="0;" } var ul="$(_3d).children(" div.tabs-header").find("ul.tabs");" _41="$(_3d).children(" div.tabs-panels");" tab="$(" <li">"+"<a href="\" javascript:;\"" class="\" tabs-inner\"">"+"<span class="\" tabs-title\""></span>"+"<span class="\" tabs-icon\""></span>"+"</a>"+"");
if(!pp){
pp=$("<div></div>");
}
...
_5a.empty();
for(var i=0;i<_56.tools.length;i++){ var t="$(" <a" href="\" javascript:;\"">").appendTo(_5a);
...
_13(_5c);
var his=[];
for(var i=0;i<_61.length;i++){ var _66="_61[i];" if(_66!="_65){" his.push(_66>_65?_66-1:_66);
}
}
_5e.selectHis=his;
...
if(_69>=0&&_69<_6b.length){ tab="_6b[_69];" if(_6a){ _6b.splice(_69,1); } }else{ var tmp="$(" <span">");
for(var i=0;i<_6b.length;i++){ var p="_6b[i];" tmp.html(p.panel("options").title); _6c="tmp.text();" tmp.html(_69); _69="tmp.text();" if(_6c="=_69){" tab="p;" if(_6a){ _6b.splice(i,1); } break; tmp.remove(); return tab; }; function _4b(_6d,tab){ _6e="$.data(_6d," tabs").tabs;" for(var i="0;i<_6e.length;i++){" if(_6e[i][0]="=$(tab)[0]){" i; -1; _24(_6f){ _70="$.data(_6f," if(tab.panel("options").tab.hasclass("tabs-selected")){ null; _71(_72){ _73="$.data(_72," tabs");" _74="_73.tabs;" _75="_74[i].panel(" options");" if(_75.selected&&!_75.disabled){ _51(_72,i); return; _51(_72,_73.options.selected); _51(_76,_77){ if(p&&!p.is(":visible")){ _78(_76); if(!p.panel("options").disabled){ p.panel("open"); _79(_7a,_7b){ if(p&&p.is(":visible")){ _78(_7a); p.panel("close"); _78(_7c){ $(_7c).children("div.tabs-panels").each(function(){ $(this).stop(true,true); }); _62(_7d,_7e){ _63(_7d,_7e)!="null;" _7f(_80,_81){ _82="$.data(_80," tabs").options;" _82.showheader="_81;" $(_80).tabs("resize"); _83(_84,_85){ _86="$(_84).find(" ">.tabs-header>.tabs-tool");
if(_85){
...
</_6b.length;i++){></_6b.length){></_61.length;i++){></_56.tools.length;i++){></0||_45></0){></_f.tools.length;i++){>