...
if($(_a).is(":visible")){
_e=_f(_c.position);
if(_c.position=="top"&&_e.top<0){ _e="_f(" bottom");" }else{ if((_c.position="=" bottom")&&(_e.top+_d._outerheight()">$(window)._outerHeight()+$(document).scrollTop())){
_e=_f("top");
}
}
if(_e.left<0){ if(_c.position="=" left"){" _e="_f(" right");" }else{ $(_a).tooltip("arrow").css("left",_d._outerwidth() 2+_e.left); _e.left="0;" } if(_e.left+_d._outerwidth()>$(window)._outerWidth()+$(document)._scrollLeft()){
