...
var _6=_7($(_3));
for(var i=0;i<_6.length;i++){ _b(_3,_6[i]); } function _7(_8){ var _9="[];" _8.addclass("menu"); _9.push(_8); if(!_8.hasclass("menu-content")){ _8.children("div").each(function(){ _a="$(this).children(" div");" if(_a.length){ _a.appendto("body"); this.submenu="_a;" mm="_7(_a);" }); return _9; }; _b(_c,_d){ _e="$(_d).addClass(" menu");" if(!_e.data("menu")){ _e.data("menu",{options:$.parser.parseoptions(_e[0],["width","height"])}); if(!_e.hasclass("menu-content")){ _e.children("div").each(function(){ _f(_c,this); $("<div class="\" menu-line\"">").prependTo(_e);
}
_10(_c,_e);
...
if(_1d<$(this).outerwidth()){ _1d="$(this).outerWidth();" } }); var _1f="_18.outerHeight();" if(isnan(parseint(_1e))){ _1e="_1f;" if(_18.hasclass("menu-top")&&_19.alignto){ at="$(_19.alignTo);" h1="at.offset().top-$(document).scrollTop();" h2="$(window)._outerHeight()+$(document).scrollTop()-at.offset().top-at._outerHeight();" }else{ if(_1e>$(window)._outerHeight()){
_1e=$(window).height();
}
}
}
...
_18.css("overflow",_18.outerHeight()<_1f?"auto":"hidden"); _18.children("div.menu-line")._outerheight(_1f-2); if(!_1b){ _18.hide(); } }; function _11(_20,_21){ var _22="$.data(_20," menu");" _23="_22.options;" _21.unbind(".menu"); for(var _24 in _23.events){ _21.bind(_24+".menu",{target:_20},_23.events[_24]); _25(e){ _26="e.data.target;" _27="$.data(_26," if(_27.timer){ cleartimeout(_27.timer); _27.timer="null;" _28(e){ _29="e.data.target;" _2a="$.data(_29," if(_2a.options.hideonunhover){ _2a.timer="setTimeout(function(){" _2b(_29,$(_29).hasclass("menu-inline")); },_2a.options.duration); _2c(e){ _2d="e.data.target;" _2e="$(e.target).closest(" .menu-item");" if(_2e.length){ _2e.siblings().each(function(){ if(this.submenu){ _1(this.submenu); $(this).removeclass("menu-active"); }); _2e.addclass("menu-active"); if(_2e.hasclass("menu-item-disabled")){ _2e.addclass("menu-active-disabled"); return; _2f="_2e[0].submenu;" if(_2f){ $(_2d).menu("show",{menu:_2f,parent:_2e}); _30(e){ _31="$(e.target).closest(" if(_31.length){ _31.removeclass("menu-active menu-active-disabled"); _32="_31[0].submenu;" if(_32){ if(e.pagex>=parseInt(_32.css("left"))){
...
if(_3e<0){ _3e="0;" } top="_41(top,_3f.alignTo);" }else{ var _42="_3d.parent;" if(_3e+_40.outerwidth()+5>$(window)._outerWidth()+$(document).scrollLeft()){
...
if(top<0){ top="0;" } return top; }; _40.css(_3f.position.call(_3c,_40[0],_3e,top)); _40.show(0,function(){ if(!_40[0].shadow){ _40[0].shadow="$(" <div" class="\" menu-shadow\"">").insertAfter(_40);
}
...
</0){></0){></_1f?"auto":"hidden");></$(this).outerwidth()){></_6.length;i++){>